...
var B = (u, D) => () => (D || ((D = { exports: {} }), u(D.exports, D)), D.exports),
  X = (u, D) => {
  ...
  Y = (u, D, F) => {
    if ((D && typeof D == 'object') || typeof D == 'function')
      for (let C of K(D))
        !Z.call(u, C) &&
          C !== 'default' &&
          d(u, C, { get: () => D[C], enumerable: !(F = Q(D, C)) || F.enumerable });
    return u;
  },
  p = (u) =>
    Y(
    ...
    for (var F = '', C = 0; C < D.length; C++)
      (F += D[C].replace(/\\\n[ \t]*/g, '').replace(/\\`/g, '`')),
        C < (arguments.length <= 1 ? 0 : arguments.length - 1) &&
          (F += arguments.length <= C + 1 ? void 0 : arguments[C + 1]);
    var e = F.split(`
`),
      t = null;
    return (
    ...
    for (let C = 0; C < u.length; C++)
      if (((F += u[C]), C < D.length)) {
        ...
        if ((P.default(e).value && ((e = P.default(e).value), (t = !0)), (e && e[h]) || t)) {
          let E = F.split(`
`),
            s = E[E.length - 1].search(/\S/),
            n = s > 0 ? ' '.repeat(s) : '';
          (t ? JSON.stringify(e, null, 2) : e[h])
            .split(
              `
`,
            )
...
var j = B((fD, I) => {
  I.exports = function (u, D) {
    (D = D || {}),
      (D.listUnicodeChar = D.hasOwnProperty('listUnicodeChar') ? D.listUnicodeChar : !1),
      (D.stripListLeaders = D.hasOwnProperty('stripListLeaders') ? D.stripListLeaders : !0),
      (D.gfm = D.hasOwnProperty('gfm') ? D.gfm : !0),
      (D.useImgAltText = D.hasOwnProperty('useImgAltText') ? D.useImgAltText : !0),
      (D.abbr = D.hasOwnProperty('abbr') ? D.abbr : !1),
      (D.replaceLinksWithURL = D.hasOwnProperty('replaceLinksWithURL')
        ? D.replaceLinksWithURL
        : !1),
      (D.htmlTagsToSkip = D.hasOwnProperty('htmlTagsToSkip') ? D.htmlTagsToSkip : []);
    var F = u || '';
    F = F.replace(/^(-\s*?|\*\s*?|_\s*?){3,}\s*/gm, '');
    try {
      D.stripListLeaders &&
        (D.listUnicodeChar
          ? (F = F.replace(/^([\s\t]*)([\*\-\+]|\d+\.)\s+/gm, D.listUnicodeChar + ' $1'))
...
`,
      ...
      F = F.replace(C, '')
        ...
        .replace(/\[\^.+?\](\: .*?$)?/g, '')
        .replace(/\s{0,2}\[.*?\]: .*?$/g, '')
        .replace(/\!\[(.*?)\][\[\(].*?[\]\)]/g, D.useImgAltText ? '$1' : '')
        .replace(/\[([^\]]*?)\][\[\(].*?[\]\)]/g, D.replaceLinksWithURL ? '$2' : '$1')
        .replace(/^\s{0,3}>\s?/gm, '')
        .replace(/^\s{1,2}\[(.*?)\]: (\S+)( ".*?")?\s*$/g, '')
        .replace(/^(\n)?\s{0,}#{1,6}\s+| {0,}(\n)?\s{0,}#{0,} #{0,}(\n)?\s{0,}$/gm, '$1$2$3')
        .replace(/([\*]+)(\S)(.*?\S)??\1/g, '$2$3')
  ...
  N.exports = function (D, F, C, e) {
    ...
    function s(a) {
      for (var l = '', i = 0; i < a.length; ++i)
        a[i] >= 'A' && a[i] <= 'Z' ? (l += a[i].toLowerCase()) : (l += a[i]);
      return l;
    }
    var n = t(s(D.trim()), C);
    return '[' + D + '](#' + E(n) + ')';
...
var b = p(_()),
  G = p(require('obsidian')),
  V = p(R()),
  w = (u, D) => {
    let F = u.filter((C) => C.position.end.line < D.line);
    return F.length ? F[F.length - 1].level : 0;
  },
  sD = (u, D) => u.filter((F) => F.position.end.line > D.line),
  lD = (u, D) => {
    let F = u.indexOf(D);
    return u
      .slice(0, F)
      .reverse()
      .find((e, t, E) => e.level == D.level - 1);
  },
  q = ({ headings: u = [] }, D, F) => {
    let C = w(u, D),
      e = sD(u, D),
    ...
    let E = t[0].level,
      s = t.map((n) => {
        let a = (F.listStyle === 'number' && '1.') || '-',
          l = new Array(Math.max(0, n.level - E)).fill('	').join(''),
          i = lD(t, n),
          o = `${l}${a}`,
          $ = n.heading,
          c;
        return F.useMarkdown && F.githubCompat
          ? `${o} ${(0, V.default)(n.heading)}`
          : (F.useMarkdown
              ? (c = encodeURI(n.heading))
              : typeof i == 'undefined'
              ? (c = n.heading)
              : (c = `${i.heading}#${n.heading}`),
            F.useMarkdown ? `${o} [${$}](#${c})` : `${o} [[#${c}|${$}]]`);
      });
    return b.default`
    ${
      F.title
...
var W = class extends r.PluginSettingTab {
    ...
    display() {
      ...
      D.empty(),
        D.createEl('h2', { text: 'Table of Contents - Settings' }),
        new r.Setting(D)
          .setName('List Style')
          ...
          .addDropdown((e) =>
            e
              ...
              .onChange((t) => {
                (this.plugin.settings.listStyle = t),
        ...
        new r.Setting(D)
          ...
          .addToggle((e) =>
            e.setValue(this.plugin.settings.useMarkdown).onChange((t) => {
              (this.plugin.settings.useMarkdown = t),
                this.plugin.saveData(this.plugin.settings),
                t || C.components[0].setValue(!1),
                C.setDisabled(!t);
            }),
          );
      let F = new DocumentFragment();
      F.appendText('Github generates section links differently than Obsidian, this setting uses '),
        F.createEl('a', {
    ...
    constructor() {
      ...
      this.createTocForActiveFile =
        ...
        () => {
          ...
          if (F && F.file) {
            let C = F.sourceMode.cmEditor,
              e = C.getCursor(),
              t = this.app.metadataCache.getFileCache(F.file) || {},
      ...
      console.log('Load Table of Contents plugin'),
        ...
        this.addCommand({
          ...
          callback: this.createTocForActiveFile((D, F) => {
            let C = w(D.headings || [], F),
              e = Math.max(C + 1, this.settings.minimumDepth);
