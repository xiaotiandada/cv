...
```html
<h1 id="test" classname="test" data-test="hello world" style="color: red;">Hello, World!</h1>
```
...
export class Component {
   ...
   */
  setProps(props: Record<string, any>, opts: { renderSync?: boolean; render?: boolean } = EMPTY) {
  ...
  render(props: object, state: object) {
    ...
    return h('div', { component: this.constructor.name }, (props as any).children);
  }
...
function isSameNodeType(node: HTMLElement & { _componentConstructor: any }, vnode: VNode): boolean {
  if (node.nodeType === 3) {
...
function build(dom: any, vnode: VNode, rootComponent?: any): HTMLElement | Text {
  let out = dom;
  let nodeName = vnode.nodeName;
